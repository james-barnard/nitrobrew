...
class Machine
  ...
  def check_component_state(id)
    ...
    if rtn_value == nil
      on_change("component #{id}".to_sym, rtn_value) { log("machine:check_component_state", "component_id: #{id.to_s}", rtn_value) }
    elsif rtn_value == false
      on_change("component #{id}".to_sym, rtn_value) { log("machine:check_component_state", "component_id: #{id.to_s}", "has timed out while #{valves[id].current_status}") }
    end
    rtn_value
  end
